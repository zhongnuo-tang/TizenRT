...
#ifndef CONFIG_AMEBAGREEN2_I2S_MAXINFLIGHT
#define CONFIG_AMEBAGREEN2_I2S_MAXINFLIGHT 32
#endif /* CONFIG_AMEBAGREEN2_I2S_MAXINFLIGHT */

/* Assume no RX/TX support until we learn better */
#undef I2S_HAVE_RX
#undef I2S_HAVE_TX

#if defined(CONFIG_AMEBAGREEN2_I2S)
...
#ifndef CONFIG_AMEBAGREEN2_I2S_DATALEN
#define CONFIG_AMEBAGREEN2_I2S_DATALEN (I2S_BITS_PER_SAMPLE_16BIT)
#endif /* CONFIG_AMEBAGREEN2_I2S_DATALEN */

#ifndef CONFIG_AMEBAGREEN2_I2S_SAMPLERATE
#define CONFIG_AMEBAGREEN2_I2S_SAMPLERATE (SR_44p1KHZ)
#endif /* CONFIG_AMEBAGREEN2_I2S_SAMPLERATE */
...
#endif /* defined(DEBUG_I2S_DRIVER) && (0 < DEBUG_I2S_DRIVER) */
#define i2serr(format, ...) printf(format, ##__VA_ARGS__)
...
struct amebagreen2_buffer_s {
	struct amebagreen2_buffer_s *flink; /* Supports a singly linked list */
...
struct amebagreen2_transport_s {
...
struct amebagreen2_i2s_config_s {
...
struct amebagreen2_i2s_s {
	struct i2s_dev_s dev; /* Externally visible I2S interface, must the first element!! */

	i2s_t *i2s_object;
	const struct amebagreen2_i2s_config_s *config;	/* Port configuration */
...
#if defined(I2S_HAVE_RX) && (0 < I2S_HAVE_RX)
	struct amebagreen2_transport_s rx; /* RX transport state */
	sem_t bufsem_rx;						 /* Buffer wait semaphore */
	struct amebagreen2_buffer_s *freelist_rx; /* A list a free buffer containers */
	struct amebagreen2_buffer_s containers_rx[I2S_DMA_PAGE_NUM];
#endif

#if defined(I2S_HAVE_TX) && (0 < I2S_HAVE_TX)
	struct amebagreen2_transport_s tx; /* TX primary transport state */
	sem_t bufsem_tx;						 /* Buffer wait semaphore */
	struct amebagreen2_buffer_s *freelist_tx; /* A list a free buffer containers */
	struct amebagreen2_buffer_s containers_tx[I2S_DMA_PAGE_NUM];
#endif
};
/* I2S device structures */
static const struct amebagreen2_i2s_config_s amebagreen2_i2s0_config = {
	.i2s_mclk_pin = I2S0_MCLK,
	.i2s_sclk_pin = I2S0_BCLK,
	.i2s_ws_pin = I2S0_WS,
	.i2s_sd_tx_pin = I2S0_SD_TX,
	.i2s_sd_rx_pin = I2S0_SD_RX,
...
/* Register helpers */
#define CONFIG_AMEBAGREEN2_I2S_DUMPBUFFERS 0

#if defined(CONFIG_AMEBAGREEN2_I2S_DUMPBUFFERS) && (0 < CONFIG_AMEBAGREEN2_I2S_DUMPBUFFERS)
...
/* Semaphore helpers */
static void i2s_exclsem_take(struct amebagreen2_i2s_s *priv);
#define i2s_exclsem_give(priv) sem_post(&priv->exclsem)

/* Buffer container helpers */
#if defined(I2S_HAVE_RX) && (0 < I2S_HAVE_RX)
static void i2s_rxdma_callback(struct amebagreen2_i2s_s *priv, int result);
static void i2s_rxdma_timeout(int argc, uint32_t arg);
static int i2s_rxdma_prep(struct amebagreen2_i2s_s *priv, struct amebagreen2_buffer_s *bfcontainer);
static int i2s_rx_start(struct amebagreen2_i2s_s *priv);
static void i2s_rx_worker(void *arg);
static void i2s_rx_schedule(struct amebagreen2_i2s_s *priv, int result);

static void i2s_bufsem_rx_take(struct amebagreen2_i2s_s *priv);
#define i2s_bufsem_rx_give(priv) sem_post(&priv->bufsem_rx)

static struct amebagreen2_buffer_s *i2s_buf_rx_allocate(struct amebagreen2_i2s_s *priv);
static void i2s_buf_rx_free(struct amebagreen2_i2s_s *priv, struct amebagreen2_buffer_s *bfcontainer);
static void i2s_buf_rx_initialize(struct amebagreen2_i2s_s *priv);

#endif

#if defined(I2S_HAVE_TX) && (0 < I2S_HAVE_TX)
static void i2s_txdma_callback(struct amebagreen2_i2s_s *priv, int result);
static void i2s_txdma_timeout(int argc, uint32_t arg);
static int i2s_tx_start(struct amebagreen2_i2s_s *priv);
static void i2s_tx_worker(void *arg);
static void i2s_tx_schedule(struct amebagreen2_i2s_s *priv, int result);

static void i2s_bufsem_tx_take(struct amebagreen2_i2s_s *priv);
#define i2s_bufsem_tx_give(priv) sem_post(&priv->bufsem_tx)

static struct amebagreen2_buffer_s *i2s_buf_tx_allocate(struct amebagreen2_i2s_s *priv);
static void i2s_buf_tx_free(struct amebagreen2_i2s_s *priv, struct amebagreen2_buffer_s *bfcontainer);
static void i2s_buf_tx_initialize(struct amebagreen2_i2s_s *priv);
...
static struct amebagreen2_i2s_s *g_i2sdevice[I2S_NUM_MAX] = {NULL};
 ...
 *
 * Input Parameters:
 *   priv - amebagreen2 private i2s data
...
static void i2s_txdma_timeout(int argc, uint32_t arg)
{
	struct amebagreen2_i2s_s *priv = (struct amebagreen2_i2s_s *)arg;
	DEBUGASSERT(priv != NULL);
...
static int amebagreen2_i2s_tx(struct amebagreen2_i2s_s *priv, struct amebagreen2_buffer_s *bfcontainer)
{
	int ret = OK;
	int *ptx_buf;
	int tx_size;
...
static int i2s_tx_start(struct amebagreen2_i2s_s *priv)
{
	struct amebagreen2_buffer_s *bfcontainer = NULL;
	int ret;
	irqstate_t flags;
	...
	bfcontainer = (struct amebagreen2_buffer_s *)sq_remfirst(&priv->tx.pend);

	/* Start first transfer */
	amebagreen2_i2s_tx(priv, bfcontainer);
...
{
	struct amebagreen2_i2s_s *priv = (struct amebagreen2_i2s_s *)arg;
	struct amebagreen2_buffer_s *bfcontainer;
	irqstate_t flags;
	...
	while (sq_peek(&priv->tx.done) != NULL) {
		...
		flags = enter_critical_section();
		bfcontainer = (struct amebagreen2_buffer_s *)sq_remfirst(&priv->tx.done);
		leave_critical_section(flags);
...
static void i2s_tx_schedule(struct amebagreen2_i2s_s *priv, int result)
{
	 ...
	 * both indirectly via the amebagreen2_dmastop() logic and directly via the
	...
	if (!sq_empty(&priv->tx.act)) {
		...
		bfcontainer = (struct amebagreen2_buffer_s *)sq_remfirst(&priv->tx.act);
...
#endif /* I2S_HAVE_TX && (0 < I2S_HAVE_TX) */
...
{
#if defined(I2S_HAVE_TX) && (0 < I2S_HAVE_TX)
	struct amebagreen2_i2s_s *priv = (struct amebagreen2_i2s_s *)dev;
	...
	/* amebagreen2 16, 24, 32, bits setting */
...
void i2s_transfer_tx_handleirq(void *data, char *pbuf)
{
	...
	struct amebagreen2_i2s_s *priv = (struct amebagreen2_i2s_s *)data;
	int tx_size;
 ...
 *
 * Input Parameters:
 *   priv - amebagreen2 i2s private data
 *   result - The result of the DMA transfer
 *
 * Returned Value:
 *   None
...
static int i2s_rx_start(struct amebagreen2_i2s_s *priv)
{
	...
	bfcontainer = (struct amebagreen2_buffer_s *)sq_remfirst(&priv->rx.pend);
...
static void i2s_rx_worker(void *arg)
{
	...
	while (sq_peek(&priv->rx.done) != NULL) {
		 ...
		 */
		flags = enter_critical_section();
		bfcontainer = (struct amebagreen2_buffer_s *)sq_remfirst(&priv->rx.done);
		leave_critical_section(flags);
...
static void i2s_rx_schedule(struct amebagreen2_i2s_s *priv, int result)
{
	...
	if (!sq_empty(&priv->rx.act)) {
		...
		bfcontainer = (struct amebagreen2_buffer_s *)sq_remfirst(&priv->rx.act);
...
#endif /* I2S_HAVE_RX && (0 < I2S_HAVE_RX)*/
...
static int i2s_stop(struct i2s_dev_s *dev, i2s_ch_dir_t dir)
{
	...
	if (dir == I2S_TX) {
		...
		amebagreen2_i2s_isr_initialize(priv);
...
/****************************************************************************
 * Name: amebagreen2_i2s_isr_initialize
 *
 * Description:
 ...
 *
 ****************************************************************************/
int amebagreen2_i2s_isr_initialize(struct amebagreen2_i2s_s *priv)
{
 ...
 *
 ****************************************************************************/
static void i2s_getdefaultconfig(struct amebagreen2_i2s_s *priv)
{
...
#ifdef CONFIG_AMEBAGREEN2_I2S_TXCL
		switch (CONFIG_AMEBAGREEN2_I2S_TXCL) {
			case 8:
 ...
 *
 ****************************************************************************/
static int i2s_allocate_wd(struct amebagreen2_i2s_s *priv)
{
#if defined(I2S_HAVE_RX) && (0 < I2S_HAVE_RX)
	if (priv->rxenab) {
...
/****************************************************************************
 * Name: amebagreen2_i2s_initialize
 *
 * Description:
 ...
 *
 ****************************************************************************/
struct i2s_dev_s *amebagreen2_i2s_initialize(uint16_t port)
{
	struct amebagreen2_i2s_config_s *hw_config_s = NULL;
	struct amebagreen2_i2s_s *priv;
	int ret;

	if (port == 0) {
		hw_config_s = (struct amebagreen2_i2s_config_s *)&amebagreen2_i2s0_config;
	} else {
		i2serr("Please select I2S0 bus\n");
		return NULL;
	}
	 ...
	 */
	priv = (struct amebagreen2_i2s_s *)kmm_memalign(64, sizeof(struct amebagreen2_i2s_s));
	if (!priv) {
		i2serr("ERROR: Failed to allocate a chip select structure\n");
		return NULL;
	}
	memset(priv, 0, sizeof(struct amebagreen2_i2s_s));
	...
	ret = amebagreen2_i2s_isr_initialize(priv);
	if (ret != OK) {
...
#ifdef CONFIG_PM
static void amebagreen2_i2s_suspend(uint16_t port)
{
	struct amebagreen2_i2s_s *priv = g_i2sdevice[port];
...
static void amebagreen2_i2s_resume(uint16_t port)
{
	struct amebagreen2_i2s_config_s *hw_config_s;
	struct amebagreen2_i2s_s *priv;
	int ret;
...
static uint32_t rtk_i2s_suspend(uint32_t expected_idle_time, void *param)
{
	...
	(void)param;
	/* Nothing to do here */
	amebagreen2_i2s_suspend(I2S_NUM_0);
...
static uint32_t rtk_i2s_resume(uint32_t expected_idle_time, void *param)
{
	...
	amebagreen2_i2s_resume(I2S_NUM_0);
