...
void wifi_event_join_status_internal_hdl(u8 *evt_info)
{
...
#ifndef CONFIG_PLATFORM_TIZENRT_OS // currently tizenrt doesnt support storing deauth
	...
	if (join_status == RTW_JOINSTATUS_DISCONNECT) {
		...
		deauth_data_pre = (struct deauth_info *)rtos_mem_zmalloc(sizeof(struct deauth_info)); // currently tizenrt doesnt support saving deauth info
