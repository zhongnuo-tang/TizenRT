...
/**
  ...
  */
USB_TEXT_SECTION
int usbd_hal_predict_next_ep(usbd_pcd_t *pcd, usbd_pcd_ep_t *ep)
{
	u32 reg;
...
int usbd_hal_config_dfifo(usbd_pcd_t *pcd)
{
	...
	UNUSED(pcd);
	usb_hal_set_rx_fifo(1024);
	usb_hal_set_np_tx_fifo(32);
	usb_hal_set_ptx_fifo(1, 16);
	usb_hal_set_ptx_fifo(2, 256);
	usb_hal_set_ptx_fifo(3, 32);
	usb_hal_set_ptx_fifo(4, 256);
	usb_hal_set_ptx_fifo(5, 128);
#else
	usbd_config_t *cfg = &pcd->config;
...
/**
  ...
  */
USB_TEXT_SECTION
int usbd_hal_ep_activate(usbd_pcd_t *pcd, usbd_pcd_ep_t *ep)
{
...
/**
  ...
  */
USB_TEXT_SECTION
int usbd_hal_ep_deactivate(usbd_pcd_t *pcd, usbd_pcd_ep_t *ep)
{
	u8 ep_num = USB_EP_NUM(ep->addr);
	u32 reg;
	u32 i;
...
/**
  ...
  */
USB_TEXT_SECTION
int usbd_hal_ep_start_transfer(usbd_pcd_t *pcd, usbd_pcd_ep_t *ep)
{
	u8 ep_num = USB_EP_NUM(ep->addr);
...
/**
  ...
  */
USB_TEXT_SECTION
int usbd_hal_ep0_start_transfer(usbd_pcd_t *pcd, usbd_pcd_ep_t *ep)
{
	u8 ep_num = USB_EP_NUM(ep->addr);
	u8 dma = pcd->config.dma_enable;
	u32 reg;
...
/**
  ...
  */
USB_TEXT_SECTION
int usbd_hal_ep_set_stall(usbd_pcd_t *pcd, usbd_pcd_ep_t *ep)
{
	UNUSED(pcd);
	u8 ep_num = USB_EP_NUM(ep->addr);
	u32 reg;
...
/**
  ...
  */
USB_TEXT_SECTION
int usbd_hal_ep_clear_stall(usbd_pcd_t *pcd, usbd_pcd_ep_t *ep)
{
	UNUSED(pcd);
	u8 ep_num = USB_EP_NUM(ep->addr);
	u32 reg;
...
/**
  ...
  */
USB_TEXT_SECTION
int usbd_hal_get_bus_status(usbd_pcd_t *pcd, u32 *bus_status)
{
	u32 result;
	u32 dsts;
	UNUSED(pcd);
