...
static int usbd_core_ep0_continue_receive_data(usb_dev_t *dev, u8 *buf, u16 len);
static int usbd_core_ep0_continue_transmit_data(usb_dev_t *dev, u8 *buf, u16 len);
static void usbd_core_parse_setup_request(usb_setup_req_t *req, u8 *buf);
#if 0
static int usbd_core_ep_flush(usb_dev_t *dev, u8 ep_addr);
#endif
static int usbd_core_handle_ep_request(usb_dev_t *dev, usb_setup_req_t  *req);
static int  usbd_core_handle_interface_request(usb_dev_t *dev, usb_setup_req_t  *req);
static int usbd_core_handle_device_request(usb_dev_t *dev, usb_setup_req_t  *req);
static int usbd_core_set_class_config(usb_dev_t  *dev, u8 config);
static int usbd_core_clear_class_config(usb_dev_t  *dev, u8 config);
...
static int usbd_core_ep_flush(usb_dev_t *dev, u8 ep_addr)
{
	return usbd_pcd_ep_flush(dev->pcd, ep_addr);
}
#endif
...
static int usbd_core_handle_interface_request(usb_dev_t *dev, usb_setup_req_t  *req)
{
	...
	//RTK_LOGD(TAG, "Interface_req:  wIndex=0x%04x bmRequestType=0x%02x bRequest=0x%02x wLength=0x%04x\n",  req->wIndex,
...
static int usbd_core_set_class_config(usb_dev_t  *dev, u8 config)
{
	...
	if ((dev->driver != NULL) && (dev->driver->set_config != NULL)) {
		/* Set configuration  and Start the Class*/
		ret = dev->driver->set_config(dev, config);
	}
...
{
	int ret = HAL_ERR_PARA;
	if ((dev->driver != NULL) && (dev->driver->clear_config != NULL)) {
		/* Clear configuration  and De-initialize the Class process*/
		ret = dev->driver->clear_config(dev, config);
	}
	return ret;
}
...
/**
  ...
  */
USB_TEXT_SECTION
int usbd_core_eopf(usb_dev_t  *dev)
{
	u8 ep_num;
	if (dev->dev_state == USBD_STATE_CONFIGURED) {
		usbd_pcd_t *pcd = dev->pcd;
